...
|---|---|---|---|---|
| P0-INT-01 | tax-engine | commit บน `feature/integrations` — wire real lint/typecheck/test scripts (เลิก echo placeholder) + tsconfig + **17 unit tests** (`src/thailand/thailand.test.ts`) สำหรับ `TaxEngine` interface + `FakeTaxEngine`/`ThailandTaxEngine`. lockfile +3 บรรทัด (vitest importer ของ tax-engine เท่านั้น — resolved อยู่แล้วในคิว tests) | — (ไม่มีจอ · package logic) · หลักฐาน gate: `pnpm --filter @juneflow/tax-engine test` = 17/17 pass (G3 fake adapter: calcWht/calcVat inclusive+exclusive · e-Tax lifecycle C4 queued→sent+void · renderRdForm placeholder · skeleton rejects) · root `typecheck` 6/6 · `build` 4/4 (CI ขั้นต่ำเขียว) | 2026-07-12 |
