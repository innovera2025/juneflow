    ...
    insert: (table: unknown) => ({
      ...
      values: (values: Record<string, unknown>) => {
        const record = (): Record<string, unknown>[] => {
          ...
          return [{ id: "new-pkg", createdAt: D0, ...values }];
        };
        return {
...
describe("PlatformWriteDb.insertOne (B-197) — INSERT allowlist + strip", () => {
  function insFake() {
    ...
    const db = {
      insert: (table: unknown) => ({
        // B-388 · BOTH insert doors (see the file-level note on stubDb). One
        // `record()` closure behind both, invoked once per DOOR CALL.
        values: (values: Record<string, unknown>) => {
          const record = (): Record<string, unknown>[] => {
            captured.push({ table, values });
            return [{ id: "srv-gen", ...values }];
          };
          return {
            returning: () => Promise.resolve(record()),
            // The awaited-directly door (plain scoped insert, no .returning()).
  ...
  // B-388 · single-recording evidence for THIS fake's door. insFake is the stub
  // the allowlist/strip assertions above read, so a door that recorded twice, or
  // that recorded on only one path, would corrupt `captured` for all of them.
  it("B-388 · records exactly +1 per write through EITHER insert door", async () => {
    const { db, captured } = insFake();
    const door = (db as unknown as {
      insert: (t: unknown) => {
    ...
    // The awaited-directly door (plain scoped insert, no .returning()).
    const bare = await door.values({ name: "bare" });
    expect(captured).toHaveLength(1);
    const ret = await door.values({ name: "ret" }).returning();
    expect(captured).toHaveLength(2);

    expect(captured).toEqual([
      { table: packages, values: { name: "bare" } },
      { table: packages, values: { name: "ret" } },
    ]);
    expect(bare).toEqual([{ id: "srv-gen", name: "bare" }]);
    expect(ret).toEqual([{ id: "srv-gen", name: "ret" }]);
  });
...
describe("PlatformWriteDb — subscriptions (B-201): UPDATE-allowed, INSERT-denied, company_id strip", () => {
  function wFake() {
    ...
    // B-388 · this door used to record NOTHING through either path, which made
    // the "INSERT is denied" test below unfalsifiable: had insertOne stopped
    // throwing and written the row instead, nothing here would have noticed.
    // Adding a `then` alone would have satisfied the AST scan and left that
    // exactly as blind (the enforce test's own blind spot 2 — it proves a `then`
    // KEY exists, not that it records). So the door now has a recorder, and both
    // paths run the same `record()` closure once per DOOR CALL.
    const ins: Array<{ table: unknown; values: Record<string, unknown> }> = [];
    const db = {
      update: (table: unknown) => ({
        set: (set: Record<string, unknown>) => ({
  ...
  // B-388 · single-recording evidence for wFake's door, and the reason it now has
  // a recorder at all: without one there is no observable to assert the denial on.
  it("B-388 · records exactly +1 per write through EITHER insert door", async () => {
    const { db, ins } = wFake();
    const door = (db as unknown as {
      insert: (t: unknown) => {
    ...
    expect(bare).toEqual([{ id: "x", name: "bare" }]);
    expect(ret).toEqual([{ id: "x", name: "ret" }]);
  });
  ...
  it("insertOne(subscriptions, …) THROWS — subscriptions is NOT in INSERT_TABLES (created at signup only)", async () => {
    ...
    // B-388: and it wrote NOTHING. Before the door had a recorder this half was
    // unassertable, so "denied" rested entirely on the throw — a version that
    // wrote the row first and threw afterwards would have passed identically.
    expect(ins).toHaveLength(0);
  });
...
// nothing admin.ts does today writes through the bare TenantDb.insert() door, so
// no assertion above changed verdict when this landed and a green suite is NOT
// evidence the conversion is right. The defect a conversion can introduce is a
// DOUBLE-count (the recording closure invoked on the way in as well as per door)
// or a second door that records somewhere else. Neither is visible to
...
//
// This file carries THREE insert doors (stubDb, insFake, wFake). The other two
// are evidenced beside their own fakes, above; this covers the module-level one.
// ===========================================================================
describe("B-388 · stubDb's two insert doors record identically, once each", () => {
  ...
  it("records exactly +1 per write and resolves identically, through EITHER door", async () => {
    ...
    const bare = await doorOf(db, packages).values({ name: "bare" });
    expect(inserted).toHaveLength(1);
    // The .returning() door (insertThrough / insert(...).returning()).
    const ret = await doorOf(db, packages).values({ name: "ret" }).returning();
    ...
    expect(bare).toEqual([{ id: "new-pkg", createdAt: D0, name: "bare" }]);
    expect(ret).toEqual([{ id: "new-pkg", createdAt: D0, name: "ret" }]);
  });
});
