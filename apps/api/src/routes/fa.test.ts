    ...
    insert: (table: unknown) => ({
      ...
      values: (values: Record<string, unknown> | Record<string, unknown>[]) => {
        const record = (): Record<string, unknown>[] => {
          ...
          return arr.map((v, i) => ({ id: v.id ?? `new-${seq++}-${i}`, createdAt: D, ...v }));
        };
        return {
...
describe("B-388 · stubDb's two insert doors record identically, once each", () => {
  ...
  it("records exactly +1 per write and resolves identically, through EITHER door", async () => {
    ...
    const bare = await doorOf(db, fixedAssets).values({ no: "bare" });
    expect(inserted).toHaveLength(1);
    // The .returning() door (insertThrough / insert(...).returning()).
    const ret = await doorOf(db, fixedAssets).values({ no: "ret" }).returning();
    expect(inserted).toHaveLength(2);

    // Both doors record the NORMALISED array — a single object becomes [obj].
    expect(inserted).toEqual([
      { table: fixedAssets, values: [{ no: "bare" }] },
      { table: fixedAssets, values: [{ no: "ret" }] },
    ]);
    // The ids prove `seq` advanced exactly ONCE per write — no door double-recorded.
    expect(bare).toEqual([{ id: "new-0-0", createdAt: D, no: "bare" }]);
    expect(ret).toEqual([{ id: "new-1-0", createdAt: D, no: "ret" }]);
  });

  it("expands an ARRAY of child rows identically through EITHER door", async () => {
    const insertedBare: Inserted[] = [];
    const bare = await doorOf(stubDb({ rows: [], inserted: insertedBare }), faAdjustments)
      .values([{ no: "a" }, { no: "b" }]);
    const insertedRet: Inserted[] = [];
    const ret = await doorOf(stubDb({ rows: [], inserted: insertedRet }), faAdjustments)
      .values([{ no: "a" }, { no: "b" }])
      .returning();

    // ONE recording for the batch (not one per row), same shape from both doors.
    expect(insertedBare).toEqual(insertedRet);
    expect(insertedBare).toEqual([{ table: faAdjustments, values: [{ no: "a" }, { no: "b" }] }]);
    expect(bare).toEqual(ret);
    // `seq` advances PER ROW inside the map (`new-${seq++}-${i}`), so a 2-row
    // batch is new-0-0 / new-1-1 — one seq tick per row, none spare, which is
    // itself the exactly-once probe for a multi-row write.
    expect(bare).toEqual([
      { id: "new-0-0", createdAt: D, no: "a" },
      { id: "new-1-1", createdAt: D, no: "b" },
    ]);
  });
});
