...
describe("B-388 · stubDb's two insert doors record identically, once each", () => {
  ...
  it("records exactly +1 per write and resolves identically, through EITHER door", async () => {
    ...
    const bare = await doorOf(db, apCreditNotes).values({ no: "bare" });
    expect(inserted).toHaveLength(1);
    // The .returning() door (insertThrough / insert(...).returning()).
    const ret = await doorOf(db, apCreditNotes).values({ no: "ret" }).returning();
    expect(inserted).toHaveLength(2);

    expect(inserted).toEqual([
      { table: apCreditNotes, values: { no: "bare" } },
      { table: apCreditNotes, values: { no: "ret" } },
    ]);
  ...
  it("expands an ARRAY of child rows identically through EITHER door", async () => {
    ...
    const bare = await doorOf(stubDb({ rows: [], inserted: insertedBare }), apCreditNotes).values([
      { no: "a" },
      { no: "b" },
    ]);
    const insertedRet: Inserted[] = [];
    const ret = await doorOf(stubDb({ rows: [], inserted: insertedRet }), apCreditNotes)
      .values([{ no: "a" }, { no: "b" }])
      .returning();
    ...
    expect(insertedBare[0]).toEqual({ table: apCreditNotes, values: [{ no: "a" }, { no: "b" }] });
