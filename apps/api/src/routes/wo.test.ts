...
describe("B-388 · stubDb's two insert doors record identically, once each", () => {
  ...
  it("records exactly +1 per write and resolves identically, through EITHER door", async () => {
    ...
    const bare = await doorOf(db, wos).values({ no: "bare" });
    expect(inserted).toHaveLength(1);
    // The .returning() door (insertThrough / insert(...).returning()).
    const ret = await doorOf(db, wos).values({ no: "ret" }).returning();
    ...
    expect(inserted).toEqual([
      { table: wos, rows: [{ no: "bare" }] },
      { table: wos, rows: [{ no: "ret" }] },
    ]);
  ...
  it("expands an ARRAY of child rows identically through EITHER door", async () => {
    ...
    const bare = await doorOf(stubDb({ rows: [], inserted: insertedBare }), wos).values([
      { no: "a" },
      { no: "b" },
    ]);
    const insertedRet: Inserted[] = [];
    const ret = await doorOf(stubDb({ rows: [], inserted: insertedRet }), wos)
      .values([{ no: "a" }, { no: "b" }])
      .returning();
    ...
    expect(insertedBare).toEqual([{ table: wos, rows: [{ no: "a" }, { no: "b" }] }]);
