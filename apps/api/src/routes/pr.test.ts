...
describe("B-388 · stubDb's two insert doors record identically, once each", () => {
  ...
  it("records exactly +1 per write and resolves identically, through EITHER door", async () => {
    ...
    const bare = await doorOf(db, prs).values({ no: "bare" });
    expect(inserted).toHaveLength(1);
    // The .returning() door (insertThrough / insert(...).returning()).
    const ret = await doorOf(db, prs).values({ no: "ret" }).returning();
    ...
    expect(inserted).toEqual([
      { table: prs, rows: [{ no: "bare" }] },
      { table: prs, rows: [{ no: "ret" }] },
    ]);
  ...
  it("expands an ARRAY of child rows identically through EITHER door", async () => {
    ...
    const bare = await doorOf(stubDb({ rows: [], inserted: insertedBare }), prs).values([
      { no: "a" },
      { no: "b" },
    ]);
    const insertedRet: Inserted[] = [];
    const ret = await doorOf(stubDb({ rows: [], inserted: insertedRet }), prs)
      .values([{ no: "a" }, { no: "b" }])
      .returning();
    ...
    expect(insertedBare).toEqual([{ table: prs, rows: [{ no: "a" }, { no: "b" }] }]);
