 ...
 *
 * Design fidelity (PLAN.md §0 rule 1): the two-crumb breadcrumb (จัดซื้อ › ใบขอซื้อ), the
 ...
 * exposes NONE of the mock's `title` (รายละเอียด), `vendor`, `requester` (ผู้ขอ), `phase`
 * (ตำแหน่งงาน), `budget %`, or `urgent` fields, so those cells render an em-dash / omit the
                ...
                {list.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={td}>
                        {/* WIRE GAP: no `urgent` flag on the wire — the "ด่วน" badge is omitted. */}
                        <span
                          className="num"
                      ...
                      {/* WIRE GAP: only need_date (need-by, not doc date) is on the wire, and no
                          created/doc date — honest em-dash for the mock's "วันที่" column. */}
                      <td style={{ ...td, color: "var(--text-3)" }}>{DASH}</td>
                      <td style={td} onClick={(e) => e.stopPropagation()}>
