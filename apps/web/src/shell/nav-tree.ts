/**
 * Sidebar nav-tree loader for @juneflow/web (P0-WEB-05).
 *
...
export interface NavSub {
  id: string;
  /** Thai nav_i18n key — passed to tn(). */
  label: string;
  /** Count-source route id for the badge pill (C10), or undefined. */
  badge?: string;
}
...
export const NAV_TREE = navData.tree as readonly NavNode[];

/** Section id -> Thai section-header key (tn()). */
export const NAV_SECTIONS = navData.sections as Readonly<Record<SectionId, string>>;

/** Narrowing helper for a Thai label string used as a tn() NavKey. */
export function asNavKey(label: string): NavKey {
  // The 15 labels absent from nav_i18n still render their Thai key verbatim via
  // tn()'s documented fallback (see BLOCKERS B-039); the cast is the single point
  // where a data-sourced Thai label enters the typed tn() surface.
  return label as NavKey;
}
...
export function navLabelForRoute(id: string): NavKey | undefined {
  ...
  return label ? (label as NavKey) : undefined;
}
