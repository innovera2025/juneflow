...
| id | label | component @ source | โครงจอ (จาก .jsx) | โครง Flutter (structural) | ภาพอ้างอิง |
|---|---|---|---|---|---|
| `srv-new` | ลูกบ้าน · แจ้งซ่อมใหม่ | `MSrvNewReport` @ mobile-screens.jsx:61 | ฟอร์ม: ยูนิต + เลือกหมวด grid 6 ช่อง + คำอธิบาย + รูป ≥1 (grid + ปุ่มเพิ่ม) + นัดวันสะดวก + ปุ่มคู่ บันทึกร่าง/ส่งแจ้งซ่อม | **BUILT** `SrvNewScreen` @ lib/screens/service/srv_new_screen.dart · POST /sales/service (money = NONE) · คำอธิบาย → `title` (คอลัมน์ free-text เดียวที่มี) · **ตัดออกเพราะไม่มีคอลัมน์รองรับ**: รูป (L94) · นัดวันสะดวก (L106) · บันทึกร่าง (L112 — machine ไม่มี draft) → BLOCKERS.md B-293 · **grid หมวด 6 ช่อง วาดครบแต่ honest-disabled และ create ไม่ส่ง `category`** — dict / seed / prototype เป็นคำคนละชุด (ตรงกันคำเดียว) และ `category` เป็น free text ที่ไม่มีใครประกาศ vocabulary ⇒ รอคำตัดสิน BLOCKERS.md B-292 | tests/visual/reference/mobile/srv-new.png |
| `srv-track` | ลูกบ้าน · ติดตามสถานะ | `MSrvTrack` @ mobile-screens.jsx:125 | การ์ดสถานะ SR + timeline 5 ขั้น + ประวัติการซ่อม + การ์ด Warranty + `MTabBar(service)` | **BUILT** `SrvTrackScreen` @ lib/screens/service/srv_track_screen.dart · GET /sales/service · timeline = state machine จริง (received→scheduled→fixing→fixed→closed) · warranty = `warranty_months_remaining` ที่ server derive · em-dash: ยูนิต · วันที่ 3 ขั้นท้าย (ไม่มีคอลัมน์) · วันส่งมอบ · **ตัดเรตติ้งในประวัติ** (ไม่มีคอลัมน์) | tests/visual/reference/mobile/srv-track.png |
| `tech-jobs` | ช่าง · รายการงาน | `MTechJobs` @ mobile-screens.jsx:198 | สถิติ 3 ช่อง + การ์ดงานตาม status (fixing/scheduled/received) พร้อมปุ่ม action ต่างกันต่อ status + `MTabBar(service)` | **BUILT** `TechJobsScreen` @ lib/screens/service/tech_jobs_screen.dart · GET /sales/service + GET /me (scope `assignee_user_id`) + POST …/schedule + …/start · ปุ่มต่อ status derive จาก `nextServiceTransition` · สถิติเหลือ 2 ช่อง (**ตัดเรตติ้ง** — ไม่มีคอลัมน์) · **ตัดปุ่ม** เลื่อนนัด/ซ่อมเสร็จ+ปิดงาน/นัดลูกค้า → BLOCKERS.md B-294 | tests/visual/reference/mobile/tech-jobs.png |
| `tech-close` | ช่าง · ปิดงาน + ลายเซ็น | `MTechClose` @ mobile-screens.jsx:263 | รูปก่อน/หลังซ่อม + รายละเอียดงาน + วัสดุที่ใช้ + กล่องลายเซ็นลูกค้า + ปุ่ม ปิดงาน+ส่งแบบประเมิน | **BUILT (thin-honest)** `TechCloseScreen` @ lib/screens/service/tech_close_screen.dart · POST …/fix + …/close · **ทั้ง 5 ช่องเป็น label + em-dash** เพราะไม่มีคอลัมน์: รูปก่อน/หลัง · รายละเอียดงาน · วัสดุ · ลายเซ็น (ไม่วาด pad — ห้ามเก็บสิ่งที่ไม่มีที่เก็บ) · CTA ตัด "+ ส่งแบบประเมิน" (close ทิ้ง rating ทิ้ง) → BLOCKERS.md B-294 | tests/visual/reference/mobile/tech-close.png |
