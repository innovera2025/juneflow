...
2. **guard ของ §1.6 ไม่ใช่ guard ของ §1.5** — guard ใน §1.5 ข้อ 4 ถามสามคำถามเรื่อง `migrate-seed`/`postgres`/`redis` เท่านั้น **มันไม่มี clause เรื่อง `api`/`web`/`edge` เลย จึงแยกเครื่องที่มี proxy กับเครื่องที่ยิง API ออกอินเทอร์เน็ตไม่ออก** — **วัดจริง: guard ของ §1.5 exit 0 ทั้งสองสภาพ** (มี public overlay และไม่มี) · บนเครื่องสาธารณะให้ใช้แบบหกข้ออ้างข้างล่าง guard กับ `up` เป็น **คำสั่งเดียว** เหมือนเดิม:
