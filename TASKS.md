...
| P0-BE-11 | backend | ready | better-auth self-host ใน Postgres + middleware tenant scope `company_id` **ทุก query** (PLAN.md §5, ภาคผนวก A) · **REWORK (ด่าน 4.5 FAIL 12 ก.ค. — security):** update() ต้อง strip/force company_id ที่ runtime เหมือน insert() + เพิ่ม G3 test ยืนยัน update reassign company_id ไม่ได้ · better-auth user-table → B-016 | P0-BE-06 | G3 (unit tenant scope — ห้ามมี query หลุด scope) | 3 ชม. |
...
| P0-WEB-04 | web | review | port-map inventory — ตาราง mapping `pototype/*.jsx` (ยกเว้นไฟล์ excluded ตาม PLAN.md §0 ข้อ 5) → โมดูล/route เป้าหมาย + ภาพอ้างอิง gallery ต่อจอ → เก็บที่ `apps/web/docs/port-map.md` · spec: pototype/ + `docs/extract/INVENTORY.md` + NAV-ROUTES.md | — | ครอบคลุมทุก .jsx ที่ไม่ถูก exclude · review โดย Wei | 3 ชม. |
