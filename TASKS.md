...
| id | เขต | สถานะ | spec pointer | dependencies | gates ที่ต้องผ่าน | ประมาณเวลา |
|---|---|---|---|---|---|---|
| P0-DEV-01 | devops | blocked | `infra/docker-compose.yml` (dev) — pg16 + redis + api + web + worker · **`docker compose up` เดียวได้ระบบ + seed** (Manifest กลุ่ม 5 ข้อกำหนด Phase 0) · **B-030: compose (เขต) แก้แล้ว แต่ build ล้ม 3 บั๊กนอกเขต — .dockerignore repo-root + root pnpm.onlyBuiltDependencies + apps/web/Dockerfile node:20→22** | P0-BE-10, P0-BE-13 | compose up สำเร็จรอบเดียว + seed ครบ + คลิกเทียบ gallery ได้ | 3 ชม. |
...
| id | เขต | สถานะ | spec pointer | dependencies | gates ที่ต้องผ่าน | ประมาณเวลา |
|---|---|---|---|---|---|---|
| P0-FIX-01 | devops | blocked | **BE-04 audit:** `ci.yml` เรียก `pnpm run --if-present test:unit/contract/e2e/visual` แต่ root package.json ไม่มีสคริปต์เหล่านี้ → CI test stage 4 ตัวเป็น no-op เงียบ · เพิ่มสคริปต์ root ที่ delegate ไป vitest/playwright จริง (root package.json ไม่ sacred · ห้ามแตะ ci.yml ที่ sacred) — **ต้องปิดก่อนเชื่อ remote CI** | — | สคริปต์รันชุด test จริง + CI stage ไม่ no-op | 2 ชม. |
