...
| id | เขต | สถานะ | spec pointer | dependencies | gates ที่ต้องผ่าน | ประมาณเวลา |
|---|---|---|---|---|---|---|
| P0-FIX-01 | platform | review | **BE-04 audit:** `ci.yml` เรียก `pnpm run --if-present test:unit/contract/e2e/visual` แต่ root package.json ไม่มีสคริปต์เหล่านี้ → CI test stage 4 ตัวเป็น no-op เงียบ · เพิ่มสคริปต์ root ที่ delegate ไป vitest/playwright จริง (root package.json ไม่ sacred · ห้ามแตะ ci.yml ที่ sacred) — **ต้องปิดก่อนเชื่อ remote CI** · **[B-032: ย้ายมาเขต platform — root package.json เป็นของ platform แล้ว]** | — | สคริปต์รันชุด test จริง + CI stage ไม่ no-op | 2 ชม. |
| P0-PLAT-01 | platform | review | **ทำ monorepo ให้ `docker compose up` build ได้ (root-side ของ B-030):** สร้าง `.dockerignore` (ตัด node_modules/.git/decks) · ตรวจ root `package.json`/`turbo.json`/`pnpm-workspace.yaml` ให้ `pnpm install --frozen-lockfile` + `pnpm --filter @juneflow/db build` รันได้ใน Docker build context (repo root) · ประสานกับ backend เรื่อง apps/api/Dockerfile (แก้เฉพาะ root files ในเขต · Dockerfile = backend) — flag ผ่าน BLOCKERS ถ้าบั๊กอยู่นอก root | — | build context จาก repo root สร้าง image ได้ (ทดสอบ `docker build` stage) | 3 ชม. |
| P0-PLAT-02 | platform | review | **tokens/i18n ownership (แทน B-011):** ตรวจ `packages/tokens` + `packages/i18n` build/typecheck/test เขียว + เป็นเจ้าของการเปลี่ยนแปลงต่อไป (BE-04/05 ทำ implementation แล้ว — platform ดูแลต่อ) · ห้ามแก้ `i18n-full.json` (sacred) | — | tokens/i18n gates เขียว | 1 ชม. |
