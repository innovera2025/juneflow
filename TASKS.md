...
| P0-FIX-03 | web | review | **WEB-06 audit:** (1) ลบ comment เท็จ 2 จุดใน `api-client.ts` (อ้าง vite proxy ที่ยังไม่มี) (2) cookie auth ขัด contract ที่ประกาศ bearer JWT → **ต้องตัดสินก่อนจอ authenticated แรก — ดู B-028** | — (B-028 ตอบแล้ว ก) | CI + comment ตรงจริง + auth mode ตรง contract | 2 ชม. |
...
| id | เขต | สถานะ | spec pointer | dependencies | gates ที่ต้องผ่าน | ประมาณเวลา |
|---|---|---|---|---|---|---|
| P1-WEB-01 | web | blocked | **จอแรก: Login** — port `ScreenLogin` จาก `pototype/extra-screens.jsx` (route `login` "เข้าสู่ระบบ") → `apps/web` · standalone (render ก่อน shell · ไม่ต้อง resource endpoints) · ทุกข้อความ = i18n key จาก i18n-full.json · สี/ระยะ = packages/tokens ห้าม hardcode · POST /auth/login ผ่าน generated client · **reference `g4/01`** · ใช้ skill `port-screen` | — | **G5 visual gate: login ตรง g4/01** + CI lint/typecheck/build + ไม่มี string นอก i18n key | 3 ชม. |
