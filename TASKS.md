...
| id | เขต | สถานะ | spec pointer | dependencies | gates ที่ต้องผ่าน | ประมาณเวลา |
|---|---|---|---|---|---|---|
| P0-FIX-01 | platform | doing | **BE-04 audit:** `ci.yml` เรียก `pnpm run --if-present test:unit/contract/e2e/visual` แต่ root package.json ไม่มีสคริปต์เหล่านี้ → CI test stage 4 ตัวเป็น no-op เงียบ · เพิ่มสคริปต์ root ที่ delegate ไป vitest/playwright จริง (root package.json ไม่ sacred · ห้ามแตะ ci.yml ที่ sacred) — **ต้องปิดก่อนเชื่อ remote CI** · **[B-032: ย้ายมาเขต platform — root package.json เป็นของ platform แล้ว]** | — | สคริปต์รันชุด test จริง + CI stage ไม่ no-op | 2 ชม. |
