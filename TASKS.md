...
| P0-FIX-02 | backend | review | **BE-09 audit:** (1) `doc_numbering` ประกาศ `unique(company,type)` ใน comment แต่ไม่มี constraint จริง → เพิ่ม unique constraint ผ่าน migration ใหม่ (2) FK 56 คอลัมน์ไม่มี index → เพิ่ม index บน FK ที่ query บ่อย · **หมายเหตุถึง BE-10 (seed): `solar_roi.cumulative` เป็นคอลัมน์จริงที่ seed ต้องเติม** | — | G1 + migration check + index มีจริง | 3 ชม. |
