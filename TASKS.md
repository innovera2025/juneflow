...
| P0-MOB-04 | mobile | review | mobile screen inventory — `pototype/mobile*.jsx` (**26 จอ distinct + host** — คำตัดสิน B-015 ก) → ตาราง widget mapping + ภาพอ้างอิง gallery mobile ต่อจอ → เก็บที่ `apps/mobile/docs/screen-map.md` (เตรียม Phase 4) | — | ครบ 26 จอ distinct + host · review โดย Wei | 3 ชม. |
...
| P1-BE-01 | backend | doing | **REWORK (ด่าน 4.5 FAIL 13 ก.ค. — 2 จุด security):** (1) `auth.ts` fallback secret ติดใน production path เงียบ (`?? DEV_ONLY_SECRET` ไม่สน NODE_ENV — ขัด infra/CLAUDE.md ห้าม secret ใน repo) → ต้อง fail-fast เมื่อ production+ไม่มี env · compose-side fix เป็นเขต devops → เปิด **B-038** (2) `TenantDb.selectReference` type-gate รั่ว — ตาราง tenant ที่ scope ผ่าน parent FK (boq_doc/work_period/subcon_contract ฯลฯ ไม่มีคอลัมน์ companyId) คอมไพล์ผ่านได้ → เพิ่ม runtime allowlist (package/project_type/company เท่านั้น) + test ครอบ + แก้ comment เกินจริง · งานส่วนอื่น (migration 0008 · seed · routes · 73 tests) reviewer ยืนยันคุณภาพสูง ห้ามแตะ · **resource routes ชุดแรก — ปลดฐานของ B-020/P0-WEB-05: implement `GET /me` + `GET /projects`** ตรง `packages/contracts/openapi.yaml` (sacred อ่านอย่างเดียว — `/me` L137 · `/projects` L357) ตอบข้อมูลจริงจาก central seed ผ่าน `TenantDb` (tenant scope ทุก query — ห้าม query หลุด scope) · auth = **bearer ตาม B-028(ก)** ต่อยอดกลไก better-auth ที่มีอยู่ (`src/auth.ts`) — ถ้าชนข้อจำกัด/ทางแยกของ better-auth ที่ spec ไม่ชี้ → BLOCKERS ห้ามเดา · error ทุกทางต้อง flat `{code,message}` ตรง contract Error · **เก็บ debt ที่ audit 12 ก.ค. ชี้ไว้ในรอบเดียวกัน (ไฟล์เดียวกัน):** (1) path จริงต้องตรง contract server `/api/v1` — live contract test ต้องไม่ 404 (2) `setNotFoundHandler` คืน flat Error (3) แก้ `routes/files.ts` 401 nested → flat · expected ของ `/me` ตรง seed จริง (ผู้ใช้ "สมชาย วัฒนกุล" — ดูบริบท B-020) · verify จริงบน compose dev (`POSTGRES_PORT=5434`) | P0-BE-10, P0-BE-13, P0-FIX-08 | **G2** live contract test ของ 2 endpoint ผ่านบน compose dev + 46 live tests เดิมยังเขียว · **G3** unit (tenant scope + shape + auth guard) · CI lint/typecheck/build เขียว | 3 ชม. |
