...
| P0-QA-03 | qa | blocked | Playwright E2E harness (`tests/e2e/`) + smoke test (login → shell load) ตาม state machine ใน `docs/handoff/flows.html` · **blocked B-034**: apps/web render แต่ Placeholder scaffold ทุก route — ยังไม่มี login form + app shell (P0-WEB-05 blocked ต่อ B-020) → smoke "login → shell load" ทำ/ผ่าน G4 ไม่ได้ ห้ามประดิษฐ์ UI behavior เอง (PLAN §0 กฎ 1/4) | P0-DEV-01 | G4 smoke ผ่านบน compose dev | 3 ชม. |
