...
| P0-BE-05 | backend | blocked | `packages/i18n` — ก๊อป `i18n-full.json` (sacred) + loader key-based โครง 3 ชั้น dict/nav/phrases (กลุ่ม 5) · spec: `docs/extract/i18n-full.json` + `docs/extract/I18N-KEYS.md` · **B-011** (packages/i18n นอก zonePaths ทุกเขต — รอ Wei) | P0-BE-01, P0-BE-02 | CI + unit loader (th/zh/en/ar+RTL) · **ห้ามแปลใหม่แม้แต่คำเดียว** | 2 ชม. |
...
| P0-BE-07 | backend | ready | schema กลุ่มโครงการ/BOQ/จัดซื้อ ตาม dictionary (+คำตัดสิน C2: basis ที่ 4 = `unit`, C3: WorkPeriod states ตาม flows/dictionary) · **REWORK — ด่าน 4.5 FAIL 12 ก.ค.:** เพิ่มตาราง `pm_quote` ตาม erd.html (id, wo_id, parts[], decision) — PLAN §6 นับ erd เป็น base schema ห้ามตัดเอง (ถ้าเห็นว่าขัดจริงให้เปิด blocker ห้ามตัดเงียบ) · migration 0001 ยังไม่ merge — regen ได้ แล้ว reset DB @5433 ก่อนรัน migrate gate | P0-BE-06 | G1 (บางส่วน) + migration check | 3 ชม. |
| P0-BE-08 | backend | ready | schema กลุ่มการเงิน-บัญชี/subscription ตาม dictionary (+C4: e-Tax superset, C5: limits key = storage_gb/ai_per_month, C9: JV lines) · **REWORK — ด่าน 4.5 FAIL 12 ก.ค.:** เพิ่มคอลัมน์ `sales_unit.contract` ตาม dictionary (ชนิดข้อมูลถาม B-013 — ห้ามอ้าง [TBD-MVP] ตัด field) · คืนคำ `contract` ใน spec-comment `src/schema/index.ts` ที่ถูกลบ · migration 0002 regen ได้ | P0-BE-06 | G1 (บางส่วน) + migration check | 3 ชม. |
