...
| P0-BE-10 | backend | blocked | `packages/db` seed — **REWORK 2 (ด่าน 4.5 FAIL: transaction data ถูกหมดแล้ว เหลือ 3 blocker ที่ตอบแล้วยังไม่ apply + defect_report):** (1) **B-021(ก)**: เขียน migration `0006` เพิ่ม enum value `trial` เข้า subscription_status + seed T-1005 (บจก. เมโทรดีเวลอปเมนท์) status=trial · ลบ map trial→expiring + comment "pending Wei" ออก (2) **B-022(ก)**: seed company 9 แถวจากชื่อ SUBSCRIBERS ใน subscription-admin.jsx:5-14 (รุ่งเรืองก่อสร้าง/สยามพร็อพเพอร์ตี้/ช่างไทยวิศวกรรม/กรีนโซลาร์/เมโทร/บ้านสวยการช่าง/นนทบุรีโยธา/ภูเก็ตวิลล่า/อีสานคอนสตรัคชั่น) · แต่ละ subscription ชี้ company ของตัวเอง (3) **B-023(ก)**: seed vendor kind=subcon 6 รายจาก subcon.jsx:4-11 + 4 คู่สัญญาจาก subcon-accept.jsx (รวม หจก.ช่างก่อฉาบมั่นคง) · subcon_contract.vendor_id ชี้ firm จริงตาม WO (4) เติม defect_report 3 แถว (schema boq.ts:332 — จาก GR rejection) หรือถ้าเห็นว่า 3 DMS docs cat=defect ครอบแล้วให้เปิด blocker ถาม Wei ห้ามตัดสินเอง (5) sync seed/index.ts header + REVIEW-QUEUE ให้เลิกบรรยาย B-021/022/023 เป็น open (ตอบแล้ว) (6) count delta (company 3→9, subcon vendor 2→8) กระทบ tests/seed ของเขต qa — flag ผ่าน REVIEW-QUEUE cross-zone ห้ามแก้ข้ามเขต · transaction rows ที่ถอด mock จริงแล้ว (BOQ/JV/work_period/bank/lead/PR ฯลฯ) **ถูกต้อง — ห้ามแตะ** | P0-BE-09 | seed รันผ่าน + จำนวน record ตรง §สรุป + persist (ไม่ reseed ทุก reload) | 3 ชม. |
