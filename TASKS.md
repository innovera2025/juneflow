...
| P0-WEB-05 | web | ready | app shell port ตรงจาก `chrome.jsx` + `shell.jsx` (sidebar/topbar/เมนู) — **ข้อมูลจริงจาก `/me` + `/projects` ผ่าน generated client** (B-020ก) · badge = count จาก query จริง (C10 · count endpoint ที่ขาด → backend blocker ห้าม hardcode) · ป้ายจาก i18n key · โครงเมนูตรง NAV-ROUTES 100% | P0-WEB-02, P0-WEB-03, P1-BE-01, P0-DEV-01 | **G5** (visual gate จอ shell เทียบ reference) | 3 ชม. |
...
| P0-DEV-06 | devops | ready | เพิ่ม `BETTER_AUTH_SECRET: ${BETTER_AUTH_SECRET:-juneflow-dev-secret}` ใน services `api` + `worker` ของ `infra/docker-compose.yml` (คำตัดสิน B-038ก — convention เดียวกับ POSTGRES_PASSWORD dev default · code คง fail-fast ใน production) | P1-BE-01 | compose up: api + worker boot ผ่านโดยไม่ต้อง set host env · G2-live re-verify P1-BE-01 บน compose | 0.5 ชม. |
